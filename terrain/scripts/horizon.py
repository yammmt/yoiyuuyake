...
def _calculate_ray(
    ...
    for distance in range(sample_interval_meters, max_distance_meters + 1, sample_interval_meters):
        sample_latitude, sample_longitude = destination_point(latitude, longitude, azimuth_degrees, distance)
        sample = elevations.elevation_at(sample_latitude, sample_longitude)
        angle = math.degrees(math.atan2(sample.elevation_meters - observer_eye_elevation_meters, distance))
